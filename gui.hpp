...
public:
    ...
    GuiEntry(const std::string& message, sf::RectangleShape shape, sf::Text text)
        : message(message), shape(shape), text(std::move(text))
    {}
};
...
public:
        ...
        GuiStyle& style, std::vector<std::pair<std::string, std::string>> entries)
    {
        ...
        {
            /* Construct the text */
            sf::Text text(*style.font);
            text.setString(entry.first);
            text.setFont(*style.font);
            text.setFillColor(style.textCol);
            unsigned int charSize = static_cast<unsigned int>(dimensions.y - style.borderSize - padding);
